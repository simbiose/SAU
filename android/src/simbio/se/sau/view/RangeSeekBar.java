 ...
 * <br />
 * Improved {@link MotionEvent} handling for smoother use, anti-aliased painting for improved anesthetics.<br />
 * <br />
...
public class RangeSeekBar<T extends Number> extends ImageView {
	 ...
	 * @param thumbImage
	 *            the {@link Bitmap} to be used as thumb
	 * @param thumbImagePressed
	 *            the {@link Bitmap} to be used as thumb when user press it
	 * @param delegate
	 *            the {@link IRangeSeekBar} with delegate methods
	 * @throws IllegalArgumentException
	 *             Will be thrown if min/max value type is not one of Long, Double, Integer, Float, Short, Byte or BigDecimal.
	 * @since {@link API#Version_2_0_0}
	 */
	public void init(T absoluteMinValue, T absoluteMaxValue, Bitmap thumbImage, Bitmap thumbImagePressed, IRangeSeekBar<T> delegate) throws IllegalArgumentException {
	 ...
	 * 
	 * @param flag
	 *            the {@link Boolean} value to be seted
	 * @since {@link API#Version_2_0_0}
	 */
		...
		case MotionEvent.ACTION_MOVE:
			if (pressedThumb != null) {
				...
				if (notifyWhileDragging && delegate != null)
					delegate.onRangeSeekBarValuesChanged(this, getSelectedMinValue(), getSelectedMaxValue());
			}
			break;
	...
	protected synchronized void onDraw(Canvas canvas) {
		...
		// draw minimum thumb
		drawThumb(normalizedToScreen(normalizedMinValue), RangeSeekBarThumb.MIN.equals(pressedThumb), canvas);

		// draw maximum thumb
		drawThumb(normalizedToScreen(normalizedMaxValue), RangeSeekBarThumb.MAX.equals(pressedThumb), canvas);
	}

	/**
	 * Overridden to save instance state when device orientation changes. This method is called automatically if you assign an id to the RangeSeekBar widget using the {@link #setId(int)} method. Other members of this class than the normalized min and max
	...
	protected void drawThumb(float screenCoord, boolean pressed, Canvas canvas) {
		canvas.drawBitmap(pressed ? thumbImagePressed : thumbImage, screenCoord - thumbHalfWidth, (float) ((0.5f * getHeight()) - thumbHalfHeight), paint);
	}
	 ...
	 * 
	 * @param normalized
	 *            the normalized value to be normalized
