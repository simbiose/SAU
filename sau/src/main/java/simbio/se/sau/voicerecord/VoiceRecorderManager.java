...
public class VoiceRecorderManager implements Runnable {
     ...
     * Default constructor
     *
     * @param filePathToSave the path to save the audio, send <code>null</code> to not save
     * @param delegate       the {@link VoiceRecorderDelegate} to send itens to
     *                       {@link VoiceRecorderManager}, can be <code>null</code>
     * @since {@link API#Version_3_1_0}
     */
    ...
    /**
     * start the {@link Thread} to get audio, the {@link Thread} to make animations and the
     * {@link Thread} to write the audio on storage
     *
     * @since {@link API#Version_3_1_0}
     */
    public void start() {
    ...
    public void run() {
        ...
        VoiceRecorderView[] voiceRecorderViews = (delegate == null
                ? null
                : delegate.getVoiceRecorderViews()
        );
        ...
        if (filePathToSave != null) {
            try {
                ...
                if (file.exists()) {
                    if (!file.delete()) {
                        delegate.onRecorderEndedWithFail(
                                new FileNotFoundException("Cannot create file")
                        );
                        return;
                    }
                }
        ...
        try {
            int readSize = AudioRecord.getMinBufferSize(
                    11025,
                ...
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        if (speakStarted)
                            ...
                            delegate.onRecorderEndedWithFail(
                                    new FileNotFoundException("Nothing spoken")
                            );
                    }
                });
            mainThread = null;
